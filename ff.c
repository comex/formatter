...
)					/* Move to zero only writes back dirty window */
{
	...
	if (wsect != sector) {	/* Changed current window */
		...
		if (fs->wflag) {	/* Write back dirty window if needed */
			if ((fs->fat_buffer != NULL) && (wsect > fs->fatbase) && (wsect < (fs->fatbase + fs->sects_fat))) {
				memcpy(fs->fat_buffer + (wsect - fs->fatbase) * 512, fs->win, 512);
				fs->fat_dirty = 1;
			} else {
...
#endif
		if (sector) {
			if ((fs->fat_buffer != NULL) && (sector > fs->fatbase) && (sector < (fs->fatbase + fs->sects_fat))) {
				memcpy(fs->win, fs->fat_buffer + (sector - fs->fatbase) * 512, 512);
			} else {
...
)
{
	...
	if (res == FR_OK) {
		...
		/* write all copies of FAT out to disk */
		int nf;
		if ((fs->fat_buffer != NULL) && (fs->fat_dirty)) {
			for (nf = 0; nf < fs->n_fats; nf++) {
				disk_write(fs->drive, fs->fat_buffer, fs->fatbase + nf * fs->sects_fat, fs->sects_fat);
			}
			fs->fat_dirty = 0;
		}
